...
description: Explicit-only independent GPT-5.6 Sol/GPT-6 Sol research with shared raw intent, matched evidence, cross-critique, and GPT-5.6 Sol adjudication.
---

# Tandem research

Use this skill only when the operator explicitly invokes `/tandem-research`,
requests GPT-5.6 Sol/GPT-6 Sol tandem research, or asks for two independent frontier
...
3. GPT-5.6 Sol `max/default` and GPT-6 Sol `max/default` receive the same raw
   prompt and the same evidence bundle. They reason independently before seeing
   the other report.
...
6. Each researcher critiques the other's completed report. GPT-5.6 Sol
   performs the final evidence-bound adjudication.
